...
var Precast = new function () {
	...
	this.enchant = function () {
		if(!Config.chantingrush) return true;
		var leader;
		var _ldrTest = false;
		
		if(me.name !== Config.Leader) {
			//me.overhead("ÿc1me.name: " + me.name + " - Config.Leader: " + Config.Leader);
		...
		if (unit) {
			do {
				/*if (me.area !== leader.area) {								// dEdits: remove this so that the chantress can chant while ldr is in town
					me.overhead("ÿc1Leader: " + "has left the building...");
					_ldrTest = true;
					break;	// stop chanting if leader left our area
				}*/
		...
		if((me.name !== Config.Leader) && _ldrTest) {
			if(!Pather.usePortal(null, leader.name)) Town.goToTown();
			return false;
		}
		// Minion
		unit = getUnit(1);
		...
		if (unit) {
			do {
				//if (me.area !== leader.area) break;	// stop chanting if leader left our area
