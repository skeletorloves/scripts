	...
	this.chatEvent = function (nick, msg) {
		...
		//
				case "andyd":
					...
					me.overhead("ÿc4Heading to Andariel's enterance...");
					d2_tele2andy();
					break;
				case "countd":
					if(me.name !== _myteleSorc) break;
				...
				case "pitd":
					if(me.name !== _myteleSorc) break;
					me.overhead("ÿc4Heading to the pit...");
					d2_pit();
					break;
				case "staffd":
					if(me.name !== _myteleSorc) break;
					me.overhead("ÿc4Heading to the maggot staff...");
					d2_telestaff();
					break;
				case "durield":
					if(me.name !== _myteleSorc) break;
				...
				case "mephd":
					if(me.name !== _myteleSorc) break;
					me.overhead("ÿc4Heading to mephy...");
					d2_telemephisto();
					break;
				case "ctrd":
					if(me.name !== _myteleSorc) break;
					me.overhead("ÿc4Heading to the centre of chaos...");
					d2_chaoscentre();
					break;
				default:
			...
			if (!me.inTown) {
				...
				if (leaderUnit) {
					if ((leader.area === me.area) && !(_tmpDist >= 0 && _tmpDist <= 70)) {					// dEdits: if flwr & ldr are in cows, but dist >=100 or undeifned, flwr goes to town
						me.overhead("ÿc7leader's distance is: " + (getDistance(me.x, me.y, leaderUnit.x, leaderUnit.y)));
		...
		//return this aftern normal
				...
				if (leaderUnit) {
					...
					if (_distTmp >= 0 && _distTmp < 70) {
						...
						me.overhead("ÿc3leader's distance is too far or unknown. clearing for now...");
...
function d2_trist() {
	me.overhead("ÿc1Stopping. Heading to trist and making a tp...");
	delay(rand(1000,1200));
	Town.goToTown(1);
	Town.move("waypoint");
...
function d2_pit() {
	me.overhead("ÿc1Stopping. Heading to pit and making a tp...");
	delay(rand(1000,1200));
	Town.goToTown(1);
	Town.move("waypoint");
...
function d2_hephasto() {
	me.overhead("ÿc1Stopping. Heading Hephasto...");
	delay(rand(1000,1200));
	Town.goToTown();
...
function d2_makemoo() {
	me.overhead("ÿc1Stopping. And making dah moo...");
	delay(rand(1000,1200));
	Town.goToTown(1);
	Town.move("stash");
	var leg, tome;
