...
function MajorTom() {
	...
	this.majortom = function (nick) {				// this.enchant
		...
		if (unit) {
			me.overhead("This is where the chanting happened...");
			/*do {
	...
	this.troops = function () {	// this.autoChant
		...
		me.overhead("This is where the I head to River of Flames, wait for troops, direct to bo, and then back to town...");
		/*var unit,
			chanted = [];
	...
	this.sendsorctochaos = function () {	// this.getLeg
		...
		if (!Config.Enchant.GetLeg) {
			...
			me.overhead("Bring the leg " + (wrongLeg ? "from this difficulty" : "") + " close to me.");
		...
		if (!portal) {
			me.overhead("Failed to enter Tristram :(");
			Town.goToTown();
		...
		Town.goToTown();
		me.overhead("Failed to get the leg :("); */
	...
	/*this.getTome = function () {
		...
		if (!akara) {
			me.overhead("Failed to buy tome :(");
	...
	/*this.openPortal = function (nick) {
		...
		if (Pather.getPortal(39)) {
			me.overhead("Cow portal already open.");

			return true;
		}

		if (me.getQuest(4, 10)) { // king dead or cain not saved
			me.overhead("Can't open the portal because I killed Cow King.");

			return false;
		}

		if (Config.Enchant.GetLeg && !me.getQuest(4, 0)) {
			me.overhead("Can't get leg because I don't have Cain quest.");
		...
		case 0: // classic
			if (!me.getQuest(26, 0)) { // diablo not completed
				me.overhead("I don't have Diablo quest.");
		...
		case 1: // expansion
			if (!me.getQuest(40, 0)) { // baal not completed
				me.overhead("I don't have Baal quest.");
		...
		me.overhead("Failed to open cow portal.");
	...
	while (true) {
		...
		if (command) {	// (command && !this.floodCheck(command))
			...
			case "watchmaker":	// "timeleft"
				...
				break;
			case Config.MJ.Triggers[0].toLowerCase(): // chant
				this.majortom(command[1]);		// this.enchant
